...
http.createServer(async (request, response) => {
  ...
  authorizeUrlInstance.searchParams.append("client_id", STRAVA_AUTHORIZE_CLIENT_ID);
  authorizeUrlInstance.searchParams.append("redirect_uri", `http://localhost:${localServerPort}`);
  authorizeUrlInstance.searchParams.append("approval_prompt", "auto");
  authorizeUrlInstance.searchParams.append("response_type", "code");
  authorizeUrlInstance.searchParams.append("scope", "read_all,profile:read_all,activity:read_all");

  authorizeState = crypto.randomBytes(16).toString("hex");
  authorizeUrlInstance.searchParams.append("state", authorizeState);

  response.writeHead(200, responseHeaders);
  response.write(`<a href="${authorizeUrlInstance.toString()}">Click to authorize</a>`);
  response.end();
